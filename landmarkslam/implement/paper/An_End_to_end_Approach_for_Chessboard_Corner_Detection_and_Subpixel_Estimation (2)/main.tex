...
% TODO: replace with actual metrics from batch_results.json
\todo[inline]{\begin{CJK}{UTF8}{gbsn}
填入 batch_evaluate_looming() 实际输出的统计指标：
MAE, RMSE, Median, Std, within 5cm/10cm/20cm, 均值相对误差等。
\end{CJK}}
...
MAE (m) & \todo{填入MAE} \\
RMSE (m) & \todo{填入RMSE} \\
Median error (m) & \todo{填入中位数} \\
Std (m) & \todo{填入标准差} \\
Mean relative error (\%) & \todo{填入相对误差} \\
$< 5$~cm (\%) & \todo{填入占比} \\
$< 10$~cm (\%) & \todo{填入占比} \\
$< 20$~cm (\%) & \todo{填入占比} \\
\bottomrule
...
% TODO: replace with actual plane estimation results
\todo[inline]{\begin{CJK}{UTF8}{gbsn}
填入 d_est vs d_gt 的 MAE/RMSE，以及 n 角度误差的均值/标准差。
\end{CJK}}
