...
pose estimation. The overall framework of our system is illustrated in Fig.~\ref{fig:framework},
which consists of two main phases: a mapping phase that detects road signs and computes their
metric 3D poses from a monocular sequence using the looming effect, and a relocalization phase
that re-identifies previously mapped signs across traversals and uses their stored poses to
construct absolute pose constraints for drift correction.

As shown in the figure, our system pipeline operates as follows: (1) a road sign is detected and its
...
pose estimate. When a previously mapped road sign is re-identified during a later traversal, this
pipeline retrieves the sign's stored 3D pose and constructs a direct absolute pose constraint that
corrects the SLAM trajectory drift---without relying on conventional feature matching or BoW
retrieval.
...
prior knowledge of the sign's physical dimensions. The geometric relationships
among the sign position, camera motion, and image expansion are illustrated in
Fig.~\ref{fig:looming_geo}.\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{images/3.1.png}
    \caption{Geometric illustration of looming-based depth recovery.}
    \label{fig:looming_geo}
\end{figure}
