...
# Windows COM 初始化（解决 Qt/RealSense 线程模式冲突）
if sys.platform == "win32":
    import ctypes
    ctypes.windll.ole32.CoInitializeEx(None, 0)  # STA → Qt 和 pyrealsense2 都能工作
import json
import time
import math
import queue
...
# GUI 常量
GPS_PLOT_W, GPS_PLOT_H = 640, 480  # 轨迹图窗口尺寸
...
class DataCollector:
    ...
    def write_gps_trajectory(self):
        ...
        cnt = len(self.gps.all_positions)
        if cnt == 0:
            print("  ⚠️  gps_trajectory.txt（无数据，仅文件头）")
        else:
            print(f"  ✅ gps_trajectory.txt: {cnt} 条")

    def _draw_gps_plot(self):
        """用 OpenCV 绘制 GPS 轨迹图"""
        if len(self.gps_lons) < 2:
            return
        lats = self.gps_lats
        lons = self.gps_lons
        margin = 0.1
        pad_lat = (max(lats) - min(lats)) * margin or 0.0001
        pad_lon = (max(lons) - min(lons)) * margin or 0.0001
        lat_min, lat_max = min(lats) - pad_lat, max(lats) + pad_lat
        lon_min, lon_max = min(lons) - pad_lon, max(lons) + pad_lon

        img = np.ones((GPS_PLOT_H, GPS_PLOT_W, 3), dtype=np.uint8) * 255
        left, right, top, bottom = 70, GPS_PLOT_W - 20, 40, GPS_PLOT_H - 40

        def to_pixel(lon, lat):
            px = left + (lon - lon_min) / (lon_max - lon_min) * (right - left)
            py = bottom - (lat - lat_min) / (lat_max - lat_min) * (bottom - top)
            return int(px), int(py)

        # 网格
        for i in range(5):
            x = left + (right - left) * i / 4
            cv2.line(img, (int(x), top), (int(x), bottom), (220, 220, 220), 1)
            y = top + (bottom - top) * i / 4
            cv2.line(img, (left, int(y)), (right, int(y)), (220, 220, 220), 1)
        ...
        # 坐标标签
        for i in range(5):
            lon_val = lon_min + (lon_max - lon_min) * i / 4
            lat_val = lat_min + (lat_max - lat_min) * (1 - i / 4)
            cv2.putText(img, f"{lon_val:.6f}", (int(left + (right - left) * i / 4) - 25, bottom + 15),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35, (80, 80, 80), 1)
            cv2.putText(img, f"{lat_val:.6f}", (5, int(top + (bottom - top) * i / 4) + 3),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35, (80, 80, 80), 1)

        # 标题
        cv2.putText(img, "GPS Trajectory", (left + 10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        # 轨迹线
        pts = [to_pixel(lon, lat) for lon, lat in zip(lons, lats)]
        for i in range(1, len(pts)):
            cv2.line(img, pts[i - 1], pts[i], (0, 100, 200), 2)

        # 当前点（红色）
        cv2.circle(img, pts[-1], 5, (0, 0, 255), -1)

        cv2.imshow("GPS Trajectory", img)
    ...
    def run(self):
        ...
        try:
            ...
            # ---- 创建显示窗口 ----
            cv2.namedWindow("Camera - D456", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("Camera - D456", 848, 480)
            cv2.namedWindow("GPS Trajectory", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("GPS Trajectory", GPS_PLOT_W, GPS_PLOT_H)
            ...
            while self.running:
                ...
                # ---- 显示摄像头画面 ----
                cv2.imshow("Camera - D456", color_img)
                ...
                if latest_gps:
                    ...
                    self.gps_lats.append(lat)
                    self.gps_lons.append(lon)
