...
// -------------------------------------------------------------------------------------
// 函数: System::TrackMonocular
// 功能: 单目模式 (含单目惯性) 的主入口函数
// -------------------------------------------------------------------------------------
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    // 检查传感器模式配置
    // TrackMonocular 只能用于 "纯单目 (MONOCULAR)" 或 "单目+IMU (IMU_MONOCULAR)" 模式
    // 如果你在构建 System 时选了 STEREO 或 RGBD，却调了这个函数，这里会报错退出
    ...
    // 步骤 2: 图像预处理
    // 复制一份图像，以免修改原图。如果配置文件要求缩放图像，则在这里进行 resize
    ...
    // 步骤 3: 检查模式切换请求 (定位模式 vs SLAM模式)
    {
        unique_lock<mutex> lock(mMutexMode);
        // 情况 A: 激活纯定位模式 (Localization Mode)
        // 这种模式下，不进行建图 (LocalMapping 停止)，只进行跟踪
        if(mbActivateLocalizationMode)
        {
            ...
            // 告诉 Tracker 只做跟踪，不插入关键帧
            mpTracker->InformOnlyTracking(true);
            mbActivateLocalizationMode = false;
        }
        // 情况 B: 停用纯定位模式 (即恢复正常 SLAM)
    ...
    // -------------------------------------------------------------------------------------
    // 步骤 4: 处理 IMU 数据 (仅限单目惯性模式)
    // -------------------------------------------------------------------------------------
    // 如果系统配置为 "单目+IMU" 模式 (IMU_MONOCULAR)
    if (mSensor == System::IMU_MONOCULAR)
        // 遍历所有传入的 IMU 测量数据 (vImuMeas)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            // 将 IMU 数据推送到跟踪器 (Tracker) 中进行预积分处理
            // 注意：IMU 数据必须在图像之前处理，以便为图像提供运动预测
            mpTracker->GrabImuData(vImuMeas[i_imu]);

    // -------------------------------------------------------------------------------------
    // 步骤 5: 处理图像数据 (核心跟踪步骤)
    // -------------------------------------------------------------------------------------
    // 调用 Tracker 的 GrabImageMonocular 函数进行单目跟踪
    // 参数：
    //   imToFeed:  传入的图像 (可能经过缩放)
    //   timestamp: 图像时间戳
    //   filename:  图像文件名(调试用)
    // 返回：
    //   Tcw: 世界坐标系到相机坐标系的变换矩阵 (即相机位姿)
...
ORB_SLAM3::Atlas* ORB_SLAM3::System::GetAtlas()
{
    return mpAtlas;
}
...
void System::SaveTrajectoryEuRoC(const string &filename, Map* pMap)
{
    ...
    if(!pMap)
    {
        cerr << "ERROR: Map is NULL" << endl;
        return;
    }
    
    // Use pMap as the map to save
    Map* pBiggerMap = pMap; 
        ...
        lend=mpTracker->mlRelativeFramePoses.end();lit!=lend;lit++, lRit++, lT++, lbL++)
    {
        ...
        Trw = Trw * pKF->GetPose()*Twb; // Tcp*Tpw*Twb0=Tcb0
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
       ...
       // pKF->SetPose(pKF->GetPose()*Two);
        ...
        else
        {
            ...
            Eigen::Quaternionf q = Twc.unit_quaternion();
            Eigen::Vector3f twc = Twc.translation();
            f << setprecision(6) << 1e9*pKF->mTimeStamp  << " " <<  setprecision(9) << twc(0) << " " << twc(1) << " " << twc(2) << " " << q.x() << " " << q.y() << " " << q.z() << " " << q.w() << endl;
        }
    }
    f.close();
}
