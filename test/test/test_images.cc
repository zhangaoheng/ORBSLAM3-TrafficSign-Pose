...
string imageDirVariable = "/home/zah/ORB_SLAM3-master/test/images"; 
...
int main(int argc, char **argv) {
    ...
    if (fs::exists(imageDir) && fs::is_directory(imageDir)) {
        ...
        cerr << "Error: Invalid image directory: " << imageDir << endl;
        return -1;
    }
    ...
    for (const string& filename : filenames) {
        ...
        Sophus::SE3f Tcw;
        try { Tcw = SLAM.TrackMonocular(frame, timestamp); } catch (...) {}
        if (!Tcw.translation().isZero()) { trajectory_data.push_back({frame_count, Tcw}); }
